...
public class PotionRing
{
	...
	private void addToTab(BuildCreativeModeTabContentsEvent event)
	{
		...
		if(event.getTabKey() == CreativeModeTabs.FOOD_AND_DRINKS)
		{
			...
			{
				event.accept(BuiltInRegistries.ITEM.get(Identifier.fromNamespaceAndPath(MODID, "ring_of_growing")).get().value());
				event.accept(BuiltInRegistries.ITEM.get(Identifier.fromNamespaceAndPath(MODID, "ring_of_shrinking")).get().value());
				event.accept(BuiltInRegistries.ITEM.get(Identifier.fromNamespaceAndPath(MODID, "ring_of_thinning")).get().value());
				event.accept(BuiltInRegistries.ITEM.get(Identifier.fromNamespaceAndPath(MODID, "ring_of_widening")).get().value());
			}
			...
			if(!list.isEmpty()) {
				...
				for (String s : list) {
					...
					if (parts.length == 2) {
						...
						final Item itemModded = BuiltInRegistries.ITEM.get(Identifier.fromNamespaceAndPath(PotionRing.MODID, "ring_of_"+id)).get().value();
