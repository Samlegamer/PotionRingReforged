...
public class PotionRingItem extends Item implements ICurioItem
{
    ...
    private void AddMobEffect(LivingEntity livingEntity, MobEffect mbEff)
    {
		if(CuriosApi.getCuriosInventory(livingEntity).resolve().isPresent()) {
			MobEffectInstance effectInstance = new MobEffectInstance(mbEff, mbEff == MobEffects.NIGHT_VISION ? 500 : 240, CuriosApi.getCuriosInventory(livingEntity).resolve().get().findCurios(this).size() - 1, true, true);
    ...
    private void reloadMobEffect(LivingEntity livingEntity, MobEffect mbEff)
	{
	    ...
	    if (livingEntity.hasEffect(mbEff)) {
			...
			if(currentMobEffect != null)
			{
                int ringAmplifier = 0;
                if (CuriosApi.getCuriosInventory(livingEntity).resolve().isPresent()) {
                    ringAmplifier = CuriosApi.getCuriosInventory(livingEntity).resolve().get().findCurios(this).size() - 1;
                }
		...
		else if (!livingEntity.hasEffect(mbEff) && CuriosApi.getCuriosInventory(livingEntity).resolve().isPresent())
		{
			if(CuriosApi.getCuriosInventory(livingEntity).resolve().get().findCurios(this).size() == 1) {
				MobEffectInstance eff = new MobEffectInstance(mbEff, baseDuration, CuriosApi.getCuriosInventory(livingEntity).resolve().get().findCurios(this).size() - 1, true, true);
