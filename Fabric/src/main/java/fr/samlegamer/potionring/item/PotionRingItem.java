...
public class PotionRingItem extends TrinketItem
{
	...
	{
		StatusEffect GROWING = Registries.STATUS_EFFECT.get(new Identifier("sizeshiftingpotions", "growing"));
		StatusEffect SHRINKING = Registries.STATUS_EFFECT.get(new Identifier("sizeshiftingpotions", "shrinking"));
		StatusEffect WIDENING = Registries.STATUS_EFFECT.get(new Identifier("sizeshiftingpotions", "widening"));
		StatusEffect THINNING = Registries.STATUS_EFFECT.get(new Identifier("sizeshiftingpotions", "thinning"));
	...
	{
		StatusEffectInstance effA1 = new StatusEffectInstance(mbEff, 240, 0, false, false);
		StatusEffectInstance effA2 = new StatusEffectInstance(mbEff, 240, 1, false, false);

		if(!livingEntity.hasStatusEffect(mbEff) && TrinketsApi.getTrinketComponent(livingEntity).get().getEquipped(this).size() == 1)
		{
			livingEntity.addStatusEffect(effA1);
		}
		else if(!livingEntity.hasStatusEffect(mbEff) && TrinketsApi.getTrinketComponent(livingEntity).get().getEquipped(this).size() == 2)
		{
